...
Introduction
-------------
    "*Keystroke dyanmics is the study of the unique timing patterns in an individual's typing, and typically includes extracting keystroke timing features such as the duration of a key press and the time elapsed between key presses.*" :cite:p:`Epp2011-rj` 
...
Each event contains information related to the specific key involved, such as the key code, character representation, and any modifier keys (e.g., Shift, Ctrl, Alt) that may be active during the event.
